...
class ShareButton extends ShareUtils {
  constructor(element, options) {
    ...
    this.config = {
      enabledNetworks: 0,
      protocol: '//',
      url: "https://adori.ai?adoriid="+adoriPlayer.adoriID,
      caption: null,
      //title: this._defaultTitle(),
      title: adoriPlayer.audioName,
      image: this._defaultImage(),
      //image: "http://carrot.is/img/fb-share.jpg",
      //description: this._defaultDescription(),
      description: adoriPlayer.audioDescription,

      ui: {
        flyout: 'sb-bottom sb-center1 sb-middle1 sb-right',
        buttonText: '',
        namespace: 'sb-',
        networkOrder: [],
        collision: false,
  ...
  _public(action) {
    ...
    if (typeof element === 'undefined')
      instances =
        ...
        super._objToArray(document.getElementsByClassName('share-button'));
    else
  ...
  _setup(element, opts) {
    ...
    // Disable adori display if not a mobile device
    if (this.config.networks.adori.enabled && !this._isMobile())
    this.config.networks.adori.enabled = false;

       // Enable pinttrest display if not a mobile device
       if (this.config.networks.pinterest.enabled==false && !this._isMobile())
       this.config.networks.pinterest.enabled = true;
  ...
  _setupInstance(instance, index) {
    ...
    for (let k in Object.keys(networks)) {
      ...
      if (typeof(network) !== "undefined") {
        ...
        if (network.className == 'email enabled')
        a.setAttribute('onclick','show_copy_dialog()');

        if (network.className == 'googlePlus enabled')
        a.setAttribute('onclick','show_embed_dialog()');
  ...
  /**
   * @method _networkAdori
   * @description Create & display a Adori window
   * @private
   */
  _networkAdori(element) {
    this._updateHref(element, 'adori://', {
      url: this.config.networks.adori.url,
      title: this.config.networks.adori.title
    });
  }
