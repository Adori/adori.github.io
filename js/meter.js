...
function updateGradient1(rangeValue) {
  const percentage =
    ((rangeValue - slider1.min) / (slider1.max - slider1.min)) * 100;
...
function updateGradient2(rangeValue) {
  const percentage =
    ((rangeValue - slider2.min) / (slider2.max - slider2.min)) * 100;
...
function updateGradient3(rangeValue) {
  const percentage =
    ((rangeValue - slider3.min) / (slider3.max - slider3.min)) * 100;
...
slider4.addEventListener("input", (e) => {
  updateGradient4(e.target.value);
}); // Update gradient oninput
...
document.getElementById("total-cost-video").innerHTML = "$ 0.08";
document.getElementById("total-cost-trans").innerHTML = "$ 0.00";
